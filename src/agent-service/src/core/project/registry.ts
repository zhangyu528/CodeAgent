...
export function addProject(targetPath: string, name?: string): ProjectInfo {
  ...
  writeProjects(data);
  console.info('[Projects] Registered project:', project.path, '->', project.name);
...
export function removeProject(path: string): void {
  ...
  if (idx !== -1) {
    ...
    writeProjects(data);
    console.info('[Projects] Removed project:', path);
  }
}
...
export function renameProject(path: string, newName: string): void {
  ...
  if (project) {
    ...
    writeProjects(data);
    console.info('[Projects] Renamed project:', path, '->', newName);
  }
}
...
export function getOrCreateDefaultProject(): ProjectInfo {
  ...
  if (!project) {
    ...
    data.projects.push(project);
    writeProjects(data);
    console.info('[Projects] Created default project:', project.path);
  }
