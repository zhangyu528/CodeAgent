...
async function getOrCreateSessionForProject(projectCwd: string): Promise<PooledSession> {
  ...
  console.info('[SessionPool] Creating AgentSession for project:', projectCwd, 'file:', sessionFile);
  ...
  if (modelFallbackMessage) {
    console.warn('[SessionPool] Model fallback:', modelFallbackMessage);
  }
...
function createSessionFile(projectCwd: string): string {
  ...
  console.info('[Agent] Created session file:', sessionFile, 'cwd:', projectCwd);
  return sessionFile;
}
...
export async function ensureAgentInitialized(): Promise<void> {
  ...
  if (!_initPromise) {
    _initPromise = (async () => {
      console.info('[Agent] Initializing...');
...
export async function activateProject(projectPath: string): Promise<void> {
  ...
  console.info('[Agent] Activated project:', projectPath);
}
...
export function getAutoCompaction(): boolean {
  return (getAgentSession() as any).autoCompactionEnabled ?? false;
}

export function isCompacting(): boolean {
  return (getAgentSession() as any).isCompacting ?? false;
}
...
export async function activateSession(sessionPath: string, _projectCwd: string): Promise<void> {
  console.log('[Pool] activateSession:', sessionPath, 'projectCwd:', _projectCwd);
  // Validate inputs - must be a non-empty string
  if (!sessionPath || typeof sessionPath !== 'string') {
    throw new Error('Invalid session path: ' + JSON.stringify(sessionPath));
  }
  ...
  try {
    ...
    if (stat.isDirectory()) {
      throw new Error('Session path is a directory, not a file: ' + sessionPath);
    }
  } catch (err: any) {
    if (err.code === 'ENOENT') {
...
export async function newGlobalSession(name?: string): Promise<NewSessionResult> {
  ...
  if (modelFallbackMessage) {
    console.warn('[Agent] Global session model fallback:', modelFallbackMessage);
  }
...
export async function newSession(name?: string): Promise<NewSessionResult> {
  ...
  if (modelFallbackMessage) {
    console.warn('[Agent] Model fallback:', modelFallbackMessage);
  }
...
export async function listAllSessions(): Promise<SessionInfo[]> {
  ...
  console.log('[SessionPool] listAllSessions, raw all sessions:', JSON.stringify(all, null, 2));
  console.log('[SessionPool] listAllSessions, total:', all.length);

  // Filter out invalid entries (directories, invalid paths)
  const validSessions = all.filter(s => {
    if (!s.path || typeof s.path !== 'string') {
      console.log('[SessionPool] FILTERED (no path):', s);
      return false;
    }
    // Must have .jsonl extension (session files)
    if (!s.path.endsWith('.jsonl')) {
      console.log('[SessionPool] FILTERED OUT (not .jsonl):', s.path, 'cwd:', s.cwd);
      return false;
    }
    return true;
  });

  // Log the valid session paths
  for (const s of validSessions) {
    console.log('[SessionPool] VALID session:', s.path, 'cwd:', s.cwd);
  }

  console.log('[SessionPool] valid sessions:', validSessions.length);

  // Group sessions by their cwd, sort by modified desc within each group
  const byCwd = new Map<string, typeof validSessions>();
  for (const s of validSessions) {
    const cwd = s.cwd || '';
...
/**
 * Get display name for a session (format: "{relative time} · {first message summary}")
 */
