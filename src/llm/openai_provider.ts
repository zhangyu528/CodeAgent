...
export class OpenAIProvider implements LLMProvider {
  ...
  constructor(opts?: { apiKey?: string; baseUrl?: string; model?: string }) {
    ...
    this.baseUrl = opts?.baseUrl || process.env.OPENAI_BASE_URL || 'https://api.openai.com/v1/chat/completions';
    this.defaultModel = opts?.model || process.env.OPENAI_MODEL || '';

    if (!this.apiKey) throw new Error('OPENAI_API_KEY is missing. Please set it in .env file.');
  ...
  async generate(messages: Message[], tools?: any[], options?: GenerateOptions): Promise<LLMResponse> {
    ...
    if (!message) throw new Error('OpenAI API Error: missing choices[0].message');
