...
import { SlashPaletteProps, HistoryPickerProps } from './types.js';
        ...
        return (
          <Box key={item.name} flexDirection="column">
            <Box>
              ...
              {hasMatch ? (
                <>
                  <Text dimColor={!isSelected} bold={isSelected}>{item.name.slice(0, matchIndex)}</Text>
                  <Text color="cyan" bold>{item.name.slice(matchIndex, matchIndex + props.query.length)}</Text>
                  <Text dimColor={!isSelected} bold={isSelected}>{item.name.slice(matchIndex + props.query.length)}</Text>
                </>
              ) : (
