...
export const VirtualMessageList = React.forwardRef<VirtualListRef, VirtualMessageListProps>(
  function VirtualMessageList({ messages, availableRows, onLoadMore, hasMoreAbove = false }, ref) {
    // ── Scroll state (single source of truth) ──────────────────────────────────
    const [scrollOffset, setScrollOffset] = useState(0);

    // ── Refs ─────────────────────────────────────────────────────────────────
    const heightCache = useRef<Map<string, number>>(new Map()); // key: message.id
    const [cacheVersion, setCacheVersion] = useState(0); // increment to trigger totalHeight recompute
    ...
    const currentMessageIds = useMemo(() => messages.map(m => m.id).join(','), [messages]);
    ...
    const totalHeight = useMemo(() => computeTotalHeight(), [computeTotalHeight, cacheVersion]);
    ...
    // ── Measure all visible items (runs after mount and after messages change) ──
    // This fills heightCache with real measurements so totalHeight is accurate
    const measureAllVisibleItems = useCallback(() => {
      let changed = false;
      for (const group of groups) {
        for (let i = group.startIndex; i <= group.endIndex; i++) {
          const msg = group.messages[i - group.startIndex]!;
    ...
    // ── After mount: measure all, then scroll to bottom ────────────────────────
    useLayoutEffect(() => {
      // Measure all items first
      measureAllVisibleItems();
      // Then scroll to bottom
      scrollToBottom();
      // eslint-disable-next-line react-hooks/exhaustive-deps
    }, []);

    // ── After messages change: re-measure, then scroll to bottom ───────────────
    useEffect(() => {
      if (messages.length === 0) return;
      // Wait for render + measure, then scroll
      const timer = setTimeout(() => {
        measureAllVisibleItems();
        scrollToBottom();
      }, MEASURE_DEBOUNCE_MS * 2);
      return () => clearTimeout(timer);
    }, [messages.length, measureAllVisibleItems, scrollToBottom]);
    ...
    return (
      <Box flexDirection="column" height={availableRows}>
        ...
        <Box flexDirection="column" height={availableRows} overflow="hidden">
          ...
          <Box flexDirection="column" marginTop={-scrollOffset} height={totalHeight}>
            {renderedItems.map(item => {
              if (item.type === 'header') {
                return (
                  <Box key={`hdr-${item.groupLabel}-${item.flatIndex}`} flexGrow={0} flexShrink={0}>
        ...
        {/* Scroll indicator */}
        {totalHeight > availableRows && <Text dimColor>{scrollPercent}%</Text>}
      </Box>
    );
  }
);
