    ...
    <ModalFrame title={props.title} width={width} footer={props.footer}>
      ...
      {windowItems.length === 0 ? (
        ...
        windowItems.map((item, idx) => {
          ...
          const checkbox = isPicked ? '[x] ' : '[ ] ';

          return (
            <Box key={`${globalIndex}-${item.value}`}>
              <Text color={isSelected ? 'cyan' : 'white'} bold={isSelected || isPicked} dimColor={!isSelected && !isPicked}>
                {padToWidth(`${prefix}${checkbox}${item.label}`, innerWidth)}
              </Text>
            </Box>
          );
        })
