  ...
  examples: [
    {
      ...
      description: 'Read a project README file',
    },
    {
      input: { filePath: '~/Documents/notes.txt' },
      description: 'Read a file using home directory shorthand',
    },
  ],
} as const;
...
export const readFileTool: AgentTool<any> = {
  ...
  execute: async (
    ...
    if (!resolvedPath.startsWith(normalizedWorkspace)) {
      return {
        content: [
          { type: 'text', text: `Error: Access denied. Path is outside workspace: ${filePath}` },
        ],
    ...
    try {
      ...
      if (stats.size > MAX_FILE_SIZE) {
        return {
          content: [
            {
              type: 'text',
              text: `Error: File too large (${Math.round(stats.size / 1024 / 1024)}MB). Maximum size is 5MB.`,
            },
          ],
      ...
      return {
        ...
        details: { filePath, success: true, size: stats.size },
      };
    } catch (error: unknown) {
