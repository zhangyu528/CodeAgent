...
| **High** | N3      | **会话生命周期与持久化基线**          | 统一 session 生命周期、异步化与原子写入，明确 `/new`、`/history`、`/resume` 语义。        | ✅ 已完成 | [详情](新架构功能需求/N3_会话生命周期与持久化基线.md)      |
| **High** | N4      | **会话存储抽象与兼容迁移**            | 引入 `SessionRepository` 抽象、schema version 与 migration，支持 JSON/SQLite 双实现路径。 | ✅ 已完成 | [详情](新架构功能需求/N4_会话存储抽象与兼容迁移.md)        |
...
| **Med**  | N12     | **自动化测试方案**                    | 建立 Vitest 单元测试体系，覆盖 slash 命令、消息处理等核心逻辑。                           | ✅ 已完成 | [详情](新架构功能需求/N12_自动化测试方案.md)               |
